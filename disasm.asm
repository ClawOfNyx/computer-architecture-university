...
.data
    ifn db 13 dup (0)   ; ivesties failo pavadinimas
    ifh dw 0    ; ivesties failo handle
    ofn db 13 dup (0)   ; isvesties failo pavadinimas
    ofh dw 1    ; isvesties failo handle

    READ_LENGTH dw 1024
    PRINT_LENGTH dw 1024

    in_buff db 1024 dup (?)
    in_buff_end dw 0    ; ivesties buferio pabaiga
    in_buff_length dw 1024  ; ivesties buferio ilgis

    eof db 0    ; zymi failo pabaiga

    out_buff db 1024 dup (?)    ; isvesties buferis
    out_buff_i dw 0

	instr_buff db 50 dup (?)
	instr_length db 0   ; nagrinejamos komandos str ilgis
	instr_pointer dw ?

    ; Disassemblerio logika
    d_val db 0
    w_val db 0
    reg_val db 0
    mod_val db 0
    ...
    open_if_error_msg db "Input file could not be opened or found$"
    create_of_error_msg db "Output file is impossible to create$"
    ...
    end_msg db "end"
    help_msg db "This is a disassembly program, that given .com file and any output file would disassemble and print the asm commands.", 0Dh, 0Ah, "Please write input and output files with extentions while launching the ptogram.", 0Dh, 0Ah, 24h
    
    ; Instrukciju konstruktoriai
    special_symbols db " ,[]:+"
    ...
    is_prefix db 0  ; nustato, ar buvo panaudotas segmentas ir jei buvo tai koks
    instructions db "MOVOUTNOTRCRXLAT" ; atpazistamos instrukcijos
    unidentified db "Unidentified command."
...
read_pars:
    ...
    ; ziurima ar reikia pagalbos
    dec cl
    mov si, 82h
    cmp word ptr es:[si], "?/"
    je do_help

    ; ieskomas ivesties failo pavadinimas
    read_ifn:
        mov di, offset ifn
    ...
    ; ziurima ar yra pateiktas isvesties failas
    end_read_ifn:
        cmp cl, 0
        je skip_read_ofn
        inc si

    ; ieskomas isvesties failo pavadinimas
    read_ofn:
        mov di, offset ofn
        read_ofn_loop:
            mov dl, es:[si]
    ...
    ; issaugom data segmento poslinki???
    skip_read_ofn:
        mov ax, ds
        mov es, ax
        xor ax, ax

; atidarom ivesties faila
open_if:
    mov ax, 3D00h
    mov dx, offset ifn
    int 21h
